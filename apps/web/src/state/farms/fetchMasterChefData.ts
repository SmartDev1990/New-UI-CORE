...
const masterChefFarmCalls = async (farm: SerializedFarm) => {
  ...
  const multiCallChainId = farmFetcher.isTestnet(quoteToken.chainId) ? ChainId.CORE : ChainId.BRISE
...
export const fetchMasterChefData = async (farms: SerializedFarmConfig[], chainId: number): Promise<any[]> => {
  ...
  const multiCallChainId = farmFetcher.isTestnet(chainId) ? ChainId.CORE : ChainId.BRISE
